...
namespace HSPI_ENVIRACOM_MANAGER
{
    ...
    [Serializable()]
	public class clsWebManager : MarshalByRefObject
	{
        public const bool HSPI_USE_ASPX = false;
        ...
        public const string PLUGIN_URL_REDIRECT = "/hspi_enviracom_manager/EnviracomConfiguration.aspx";

        public string GenPageAspx(ref string lnk)
        {
            ...
            pag.Append("<HTML>");
            pag.Append("<HEAD>");
            pag.Append("<title>Enviracom Manager Configuration</title>");
            pag.Append("<meta http-equiv=\"refresh\" content=\"0;url=" + PLUGIN_URL_REDIRECT  + "\"");
            pag.Append("</HEAD>");
            pag.Append("<BODY>");
            pag.Append("<H2>");
            pag.Append("Please wait...loading enviracom manager configuration");
            pag.Append("</H2>");
            pag.Append("</BODY>");
            pag.Append("</HTML>");

            hdr.Append("HTTP/1.0 200 OK");
            hdr.Append("Server: HomeSeer");
            hdr.Append("Content-Type: text/html");
            hdr.Append("Accept-Ranges: bytes");
            hdr.Append("Content-Length: " + pag.Length + "\n");
            hdr.Append(pag.ToString());

            return hdr.ToString();
        }

        public DataSet ThermostatDataSet()
        {
            DataSet dsThermoStats = new DataSet();
            dsThermoStats.Tables.Add(ConvertTo<clsThermostatZone>(m_objApp.Thermostats));
            return dsThermoStats;
        }

        public DataSet UnitDataSet()
        {
            DataSet dsUnits = new DataSet();
            dsUnits.Tables.Add(ConvertTo<clsHvacController>(m_objApp.Units));
            return dsUnits;
        }

        public DataSet ScheduleDataSet()
        {
            DataSet dsSchedules = new DataSet();
            dsSchedules.Tables.Add(ConvertTo<clsScheduleDay>(m_objApp.Schedules));
            return dsSchedules;
        }
        ...
        public string BuildThermoPage()
        {
            ...
            p.Append("<tr><td class=\"tableheader\" colspan=\"20\">Enviracom&nbsp;Thermostat&nbsp;Status</td></tr>");
